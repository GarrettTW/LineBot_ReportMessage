...
def handle_message(event):
    ...
    if '姓名' in receivedmsg and '學號' in receivedmsg and '手機' in receivedmsg:
        ...
        try:
            LineMessage = (
                '完成回報的號碼有:\n'
                +str([number for number in sorted(ReportData.keys())]).strip('[]')
            )
        ...
        try:
            LineMessage = ''
            for data in [ReportData[number] for number in sorted(ReportData.keys())]:
